...
void MAX86150::bitMask(uint8_t reg, uint8_t mask, uint8_t thing)
{
  ...
  uint8_t current = readRegister8(_i2caddr, reg);

  // Prepare mask: keep bits outside mask, and update bits within mask with 'thing'
  uint8_t preserved = current & mask;          // bits we keep
  uint8_t updated   = preserved | (thing & ~mask); // combine with new bits (ensure we only set bits intended)

  // Only write back if something changes to reduce I2C traffic
  if (updated != current) {
    writeRegister8(_i2caddr, reg, updated);
  }
}

uint8_t MAX86150::readRegister8(uint8_t address, uint8_t reg) {
  // Request one byte from 'reg' over I2C in a robust way
  _i2cPort->beginTransmission(address);
  _i2cPort->write(reg);
  uint8_t result = 0;
  if (_i2cPort->endTransmission() == 0) {
    _i2cPort->requestFrom((uint8_t)address, (uint8_t)1);
    // Use a small timeout loop to wait for data instead of blocking indefinitely
    unsigned long start = millis();
    while (_i2cPort->available() == 0 && (millis() - start) < 5) { /* wait up to 5 ms */ }
    if (_i2cPort->available()) {
      result = _i2cPort->read();
      return result;
    }
  }
  // If we reach here, return 0 as default (caller should handle invalid values)
  return 0;
}
